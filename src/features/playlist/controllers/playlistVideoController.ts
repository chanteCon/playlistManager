import { AuthRequest } from 'features/auth/types';
import { PlaylistService } from '../services/playlistService';
import { Response } from 'express';
import { PlaylistIdParams, PlaylistVideoReference, PlaylistVideoUpdateInput } from '../types';
import { canSendResponse } from 'shared/helper';

export type PlaylistVideoController = ReturnType<typeof createPlaylistVideoController>;
export const createPlaylistVideoController = (playlistService: PlaylistService) => {
    const addVideo = async (
        req: AuthRequest<PlaylistIdParams, any, { url: string }>,
        res: Response,
    ) => {
        const userId = req.user!.id;
        const { id } = req.params;
        const video = await playlistService.addVideo(userId, { playlistId: id, url: req.body.url });
        return res.status(201).json({ video });
    };
    const updateVideo = async (
        req: AuthRequest<PlaylistVideoReference, any, PlaylistVideoUpdateInput>,
        res: Response,
    ) => {
        const userId = req.user!.id;
        const { id, playlistVideoId } = req.params;
        const video = await playlistService.updateVideo(userId, {
            playlistId: id,
            playlistVideoId: playlistVideoId,
            data: req.body,
        });
        return res.status(200).json({ video });
    };
    const deleteVideo = async (req: AuthRequest<PlaylistVideoReference>, res: Response) => {
        const { id, playlistVideoId } = req.params;
        const userId = req.user!.id;
        await playlistService.removeVideo(userId, {
            playlistId: id,
            playlistVideoId: playlistVideoId,
        });
        if (canSendResponse(res)) {
            return res.status(204).send();
        }
    };

    return { addVideo, updateVideo, deleteVideo };
};
